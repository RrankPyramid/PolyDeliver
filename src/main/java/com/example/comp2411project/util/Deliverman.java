...
public class Deliverman implements Table {
    ...
    public Table pushInfo() throws SQLException{
        ...
        boolean hasValue = oracleDB.existValue("DELIVERMAN", "DELIVERMANID", delivermanId);
        if(hasValue){
            oracleDB.update("UPDATE DELIVERMAN "+
                    "SET USERNAME = ?, PASSWORD = ?, PHONENO = ?, ORDERNO = ?, PX = ?, PY = ? "+
                    "WHERE DELIVERMANID = ?", username, password, phoneNO, orderNO, px, py, delivermanId);

        }else{
            delivermanId = oracleDB.insert("INSERT INTO DELIVERMAN(USERNAME, PASSWORD, PHONENO, ORDERNO, PX, PY) VALUES(?, ?, ?, ?, ?, ?)","DELIVERMANID", username, password, phoneNO, orderNO, px, py);
        }
    ...
    public Table pullUpdate(){
        OracleDB oracleDB = OracleDB.getInstance();
        oracleDB.getConnection();
        try(ResultSet rs = oracleDB.query("SELECT USERNAME, PASSWORD, PHONENO, ORDERNO, PX, PY FROM DELIVERMAN WHERE DELIVERMANID = ?", delivermanId)){
            if(rs.next()){
    ...
    public Customer getNowCustomer() throws UnsupportedOperationException{
        if(orderNO == null)
    ...
    public static long checkUsernameAndPassword(String username, String password) throws IllegalArgumentException{
        ...
        try(ResultSet rs = oracle.query("SELECT PASSWORD, DELIVERMANID FROM DELIVERMAN WHERE USERNAME = ?",  username)){
            if(rs.next()){
