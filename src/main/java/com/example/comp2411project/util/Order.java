...
public class Order implements Table{
    ...
    public Table pushInfo() throws SQLException{
        ...
        if(hasValue){
            ...
            orderId = oracleDB.insert("INSERT INTO ORDERS(MERCHANTID, CUSTOMERID, PRICE, STATUS) VALUES(?, ?, ?, ?)", "ORDERID", merchantID, customerID, price, status);
            for(long goodID : goodIDs){
                oracleDB.insert("INSERT INTO ORDER2GOODS(ORDERID, GOODSID) VALUES(?, ?)", "ORDERID", orderId, goodID);
            }
        }
    ...
    public Table pullUpdate(){
        ...
        try{
            ...
            if(goodIDs == null){
                goodIDs = new TreeSet<>();
                rs = oracleDB.query("SELECT GOODSID FROM ORDER2GOODS WHERE ORDERID = ?", orderId);
                while (rs.next()){
